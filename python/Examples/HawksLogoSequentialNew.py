...
def cost(x, vidx, params):
    ...
    if vidx:
        y = reshape(x, params.traj, params.inipts[vidx, :], params.finalpts[vidx, :])
        nveh = y.shape[0] // 3
        dist = 0.0
        traj = Bernstein(y[-3:, :])
        ...
        for i in range(nveh-1):
            ...
            # dist += 1000/dv.normSquare().integrate()
            # dist -= sum(np.log(dv.normSquare().elev(DEG_ELEV).cpts.squeeze()/1000))
...
@njit(cache=True)
def reshape(x, traj, inipt, finalpt):
    ...
    :rtype: np.ndarray
    """
    NDIM = 3    # Made for 3D problems only
...
def initguess(vidx, params):
    ...
    for d in range(params.ndim):
        ...
        x0[idx:idx+params.deg-1] = np.linspace(params.inipts[vidx, d],
                                               ...
                                               params.deg+1)[1:-1] + np.random.randn(params.deg-1)
...
if __name__ == '__main__':
    ...
    for j in range(10):
        ...
        for i in range(100):
            ...
            if i == 0:
                traj = reshape(x0, traj, params.inipts[i, :], params.finalpts[i, :])
            else:
                def _fun(x): return cost(x, i, params)
                while True:
                    try:
                        res = minimize(_fun,
                                       x0,
                ...
                traj = reshape(res.x, traj, params.inipts[i, :], params.finalpts[i, :])
